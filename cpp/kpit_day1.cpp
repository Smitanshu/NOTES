/* what is oops ?
=>
Which is parent class of all objects?
=>
At the time of object created which memory is alloted= ram.

Modular Programming:In this the procedural program is divided int o functions and each functions has clear purpose.
Better than procedural programming
suitable for large programming.     void main(){ void function()     void function()}

problems in modular programming : Data remains alive within module , so we need some data to global.
->in large project difficult to conceptualize and modify.


while creating object --.compiler will refer to class for (memory allocation).

class=(blueprint) does not take the memory the object takes the memory



how you differentiate and c++ ?
=>

string-user defined datatype of a class


Access specifier is used for which concept ?
=> to maintaning the security and data hiding.

how the polymorphism helps for developer ?
=>perform reusability ,
...
->getter setter always helps to create project.
...
// computes the root of quadratic equation
